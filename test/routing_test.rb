...
class RoutingTest < Minitest::Test
  ...
  def test_vrp_starts_ends
    ...
    manager = ORTools::RoutingIndexManager.new(data[:distance_matrix].length, data[:num_vehicles], data[:starts], data[:ends])
    routing = ORTools::RoutingModel.new(manager)
    ...
    assert_equal [16, 14, 13, 12, 11, 0], routes[3]
    assert_equal 1346, distances[3]

    assert_equal 1346, max_route_distance
  end
  ...
  def test_initial_routes
    ...
    initial_solution = routing.read_assignment_from_routes(data[:initial_routes], true)

    search_parameters = ORTools.default_routing_search_parameters
    solution = routing.solve_from_assignment_with_parameters(initial_solution, search_parameters)
    ...
    assert_equal [0, 9, 10, 16, 14, 0], routes[0]
    assert_equal 1552, distances[0]

    assert_equal [0, 12, 11, 15, 13, 0], routes[1]
    assert_equal 1552, distances[1]

    assert_equal [0, 3, 4, 1, 7, 0], routes[2]
    assert_equal 1552, distances[2]

    assert_equal [0, 5, 2, 6, 8, 0], routes[3]
    assert_equal 1552, distances[3]
